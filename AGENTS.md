...
`public/images/great-wave.png` is a supplied illustration, not generated by the script above — it's currently unused (it was the home page background until that was removed for a plainer hero) but kept in case it's wanted again. If it's ever reused as a background: check for a real alpha channel first (`sharp(...).metadata()` → `hasAlpha`); a "transparent" area rendered as a visible checkerboard usually means it's baked into the pixels, not real alpha, and needs masking (e.g. a circle via `sharp` + an SVG mask, `blend: 'dest-in'`) rather than color-keying the checker pattern out.
